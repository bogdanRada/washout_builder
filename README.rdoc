...
Type the following from the command line to install:
  ...
  gem install washout_builder ( it will automatically install  also wash_out version 0.9.0 that is currently used)
...
Also this gem contains another feature about using exceptions. I am going to give an example.
If you have two applications . The first one that acts like a webservice server that waits for calls from the other application wich is a soap client.
Let's say your first applications needs to send a error code for a specific exception, everytime that happens, so that the client application can treat it in a general way every time it happens
...
and then when you raise it in the code you can do this raise MyException.new (1001, "damn! found a error") . The first argument can be a integer or a string representing a faultcode that both the webservice and the client know about.
And the second is a message for the error. This exception is going to be catched by the gem and is going to send it to the client with the specified fault code and message
...
You can easily do that by passing a third argument an array that contains hashes in this form
 ...
 { :related => "the object's field name that is not valid",
    ...
    :arguments => "possible arguments"             # this doesn't necessarily need to be string, it can be array or hash . but it can't be a class ( You could use this for Internalization between the two applications)
}
